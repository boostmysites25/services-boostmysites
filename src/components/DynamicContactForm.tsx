...
const DynamicContactForm: React.FC<DynamicContactFormProps> = ({ 
  ...
  const demoData = demoMode ? {
    salespersonData: {
      display_name: 'Demo Salesperson',
      email: 'demo@boostmysites.in'
    },
    serviceData: {
  ...
  const finalData = demoMode ? demoData : {
    salespersonData,
    serviceData,
    ref
  };
  ...
  const getAccentStyling = (color: string) => {
    switch (color) {
      ...
      case 'purple':
        return {
          textGradient: 'from-indigo-300 via-purple-500 via-indigo-400 to-purple-400',
  ...
  return (
    <section className={`py-20 ${backgroundColor === 'transparent' ? 'bg-black/80' : 'bg-gradient-to-b from-black to-gray-900'} ${className}`}>
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-8">
          <h2 className={`text-3xl font-bold bg-gradient-to-r ${styling.textGradient} bg-clip-text text-transparent mb-4`}>
            Get in Touch with {finalData.salespersonData.display_name}
          </h2>
          <p className="text-gray-300 text-lg">
            Ready to start your {finalData.serviceData.name} project? Fill out the form below and{" "}
            {finalData.salespersonData.display_name} will get back to you within 24 hours.
          </p>
          {finalData.ref && (
          ...
          {demoMode && (
            <p className={`text-xs text-${accentColor}-400 mt-2`}>
              🧪 Demo Mode - This form demonstrates the dynamic contact form functionality
            </p>
          )}
        </div>
        ...
        <SalespersonContactForm
          ...
          sourcePage={demoMode ? 'demo-mode' : `${salesperson}-${service}-${ref || 'direct'}`}
