  ...
  useEffect(() => {
    const fetchPortfolioProjects = async () => {
      try {
        ...
        console.log(
          "WebAppsPage - Web app projects loaded:",
          webAppProjects.length
        );
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {portfolioProjects.map((project, index) => (
              <div
                key={project.id}
                className="group bg-gray-800/50 rounded-2xl overflow-hidden border border-gray-700/50 hover:border-cyan-400/50 transition-all duration-300"
              >
