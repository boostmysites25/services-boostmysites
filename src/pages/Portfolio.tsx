    ...
    const loadPortfolioData = async () => {
      try {
        ...
        const totalProjects = data.reduce(
          (total, service) => total + service.projects.length,
          0
        );
        ...
        const craveKitchenProject = allProjects.find((p) =>
          p.title.toLowerCase().includes("crave kitchen")
        );
        if (craveKitchenProject) {
          console.log("Portfolio Page - ✅ Crave Kitchen found in loaded data");
        } else {
          console.log(
            "Portfolio Page - ❌ Crave Kitchen NOT found in loaded data"
          );
        }
